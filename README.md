...
- 支持并发下载、任务进度和实时日志
- 支持自定义下载目录（默认用户主目录）、可选 Cookie

## 环境要求

- Windows 10/11 x64
- Node.js 18+（当前代码无第三方依赖）
- PowerShell 5.1+（用于下载 BBDown 与打包 Electron）
...
3. 视需要设置搜索参数（排序、时长、tid、发布时间、候选上限）。  
4. 点击“开始批量下载”。  
5. 在右侧查看每首歌的状态、命中 BVID、评分和日志。  
...
- `scripts/setup-bbdown.ps1` 与 `scripts/build-electron-portable.ps1` 默认会校验 GitHub release 资产 SHA256（来自 release asset digest）。  
...
说明：打包脚本会校验 Electron 压缩包 SHA256（来自 GitHub release asset digest），校验失败会终止打包。
